...
onready var mapSize = [15, 10]

onready var inputs = [{"coords": Vector2(0,1), "key": "key", "keyType": "string"},
					{"coords": Vector2(0,2), "key": 100, "keyType": "integer"}]

onready var locks = [{"coords": Vector2(1,1), "key": "key"},
					{"coords": Vector2(1,2), "key": 100}]

onready var ports = [{"from": Vector2(2,2), "to": Vector2(3,4)}]
